...
from youtube_extractor import get_info
...
if selection == "All workouts":
    ...
    for wo in workouts:
        url = "https://youtu.be/" + wo["vid_id"]
        st.text(wo['title'])
        st.text(f"{wo['channel']} - {get_duration_text(wo['duration'])}")
        
        ok = st.button('Delete workout', key=wo["vid_id"])
        if ok:
            dbs.delete_workout(wo["vid_id"])
